...
fn get_factures(
    ...
    let order_by = if sort_by == "oldest" {
        "created_at ASC"
    } else {
        "created_at DESC"
    };
    ...
    let mut count_stmt =
        conn.prepare(&format!("SELECT COUNT(*) FROM factures {}", where_clause))?;
    let total_count: i64 = count_stmt.query_row([], |row| row.get(0))?;

    let mut stmt = conn.prepare(&format!(
        "SELECT * FROM factures {} ORDER BY {} LIMIT ?1 OFFSET ?2",
...
fn get_settings(state: tauri::State<AppState>) -> Result<HashMap<String, String>> {
    ...
    let settings_iter = stmt.query_map([], |row| Ok((row.get(0)?, row.get(1)?)))?;
...
fn get_theme(state: tauri::State<AppState>) -> Result<Theme> {
    ...
    let styles_str: String = conn
        .query_row("SELECT styles FROM theme WHERE id = 1", [], |row| {
            row.get(0)
        })
